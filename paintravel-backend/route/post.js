...
router.post("/getPostList", async (req, res) => {
  let post = await Post.find({ writer: req.body.currentId, nationCode: req.body.selectCountry }).select();
  let postList = [];
  for (i in post) {
...
router.post("/getPostInfo", async (req, res) => {
  ...
  let post = await Post.find({ writer: req.body.currentId, nationCode: req.body.selectCountry, _id: req.body.post_id }).select();
...
router.post("/getPostEdit", upload.array("myfile"), async (req, res) => {
  console.log("글수정하러 옴")
  let postWritedInfo = await Post.find({ _id: req.body.post_id }).select("writer file");
  if (postWritedInfo.writer === req.body.currentId) {
    ...
    let post = await Post.findByIdAndUpdate(req.body.post_id, editedPost, { new: true }).exec();
    if (!post) return res.state(400).json({ postEditSuccess: false, errMessage : "DB데이터 수정 실패"});
    return res.status(200).json({
      postEditSuccess: true,
      editedPostInfo: post,
    });
  }else{
    return res.state(400).json({ postEditSuccess: false, errMessage : "작성자가 일치하지 않습니다"});
  }
});
module.exports = router;
...
router.post("/getPostDelete", async (req, res) => {
  ...
  if (postWriterInfo.writer === req.body.currentId) {
    ...
    return res.status(200).json({ postDeleteSuccess: true, errMessage : "DB데이터 수정 실패"});
  }else{
    return res.state(400).json({ postEditSuccess: false, errMessage : "작성자가 일치하지 않습니다"});
  }
});
